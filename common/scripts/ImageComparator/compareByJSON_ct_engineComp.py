...
def get_pixel_difference(work_dir, base_dir, img, baseline_json, tolerance, pix_diff_max):
    if 'render_color_path' in img.keys():
        ...
        if not os.path.exists(baseline_img_path.replace('second', 'first')):
            core.config.main_logger.warning("Original Baseline image not found by path: {}".format(baseline_img_path))
        else:
            img.update({'original_color_path': os.path.relpath(baseline_img_path.replace('second', 'first'), work_dir)})
            for thumb in core.config.THUMBNAIL_PREFIXES:
                if thumb + baseline_name in baseline_json.keys() and os.path.exists(os.path.join(base_dir, baseline_json[thumb + baseline_name]).replace('second', 'first')):
                    img.update({thumb + 'original_color_path': os.path.relpath(os.path.join(base_dir, baseline_json[thumb + baseline_name]).replace('second', 'first'), work_dir)})
...
def get_rendertime_difference(base_dir, img, time_diff_max):
    manifest_path = os.path.join(base_dir, img['test_group'], core.config.BASELINE_REPORT_NAME)
    ...
    if os.path.exists(manifest_path.replace('second', 'first')):
        with open(manifest_path, 'r') as file:
            baseline_report_json = json.loads(file.read())
            try:
                baseline_time = [x for x in baseline_report_json if x['test_case'] == img['test_case']][0][
                    'render_time']
            except IndexError:
                baseline_time = -0.0

        img.update({'difference_time_or': get_diff(render_time, baseline_time)})
        img.update({'or_render_time': baseline_time})
    else:
        img.update({'difference_time_or': -0.0})
        img.update({'or_render_time': -0.0})
    return img


def main(args):
    args.base_dir = os.path.join(args.base_dir, 'second')
