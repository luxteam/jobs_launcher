    ...
    if 'render_color_path' in img.keys():
        ...
        if 'thumb256_render_color_path' in baseline_json:
            ...
            if os.path.exists(thumb256_baseline_img_path):
                img.update({'thumb256_baseline_color_path': os.path.relpath(thumb256_baseline_img_path, work_dir)})
            else:
        ...
        if 'thumb64_render_color_path' in baseline_json:
            ...
            if os.path.exists(thumb64_baseline_img_path):
                img.update({'thumb64_baseline_color_path': os.path.relpath(thumb64_baseline_img_path, work_dir)})
            else:
                core.config.main_logger.warning("Can't find {}".format(os.path.join(base_dir, img['test_group'], baseline_json['thumb64_render_color_path'])))
